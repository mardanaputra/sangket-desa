...
export default function Navbar({ scrolled }) {
  // 1. Tambahkan state mounted untuk memastikan komponen sudah dimuat di browser
  const [mounted, setMounted] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const pathname = usePathname();

  // 2. Gunakan useEffect untuk mengubah mounted menjadi true setelah render pertama
  useEffect(() => {
    setMounted(true);
  }, []);
        ...
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="flex items-center gap-12 flex-grow justify-end">
            ...
            <div className="hidden md:flex">
              {/* Gunakan mounted check jika nanti ada logika login (localStorage) */}
          ...
          <div className="flex md:hidden">
            <button
              ...
              aria-label="Buka Menu" // Tambahan aksesibilitas
            >
...
function NavLink({ href, label, scrolled, active }) {
  ...
  // Hapus drop-shadow saat scrolled agar lebih bersih
  const shadowClass = scrolled ? "" : "drop-shadow-lg";
